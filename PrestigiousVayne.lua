...
function Interrupt()
	...
	for k,v in pairs(t) do
		...
		if not IsDead(v) and IsEnemy(v) and GetDistanceSqr(targetPos, myHeroPos) < GetMyRange() * GetMyRange() then
			if IsCasting(v) then
				DelayAction(function()
					if IsCasting(v) and GetDistanceSqr(target.pos, myHeroPos) < GetMyRange() * GetMyRange() then
						CastSpellTarget(v, E)
					end
				end,0.5)
			end
...
-------------------------------------<Main Script>-------------------------------------
--[[	
local menu = menuInst.addItem(SubMenu.new("Prestigious Vayne"))

         menu.addItem(MenuSeparator.new("   General Features", true))
         menu_combo =  menu.addItem(SubMenu.new("Combo"))
         menu_combo.addItem(MenuSeparator.new("   Spell Settings", true))
         menu_combo_q =  menu_combo.addItem(MenuBool.new("Use Q", true))
         menu_combo_w =  menu_combo.addItem(MenuBool.new("Use W", true))

         menu.addItem(MenuSeparator.new("   Advanced Features", true))
        -- menu_condemn = menu.addItem(MenuStringList.new("Condemn mode", {"condemn1", "condemn2"} , 1))
         menu_keys =  menu.addItem(SubMenu.new("Keybinds"))
         menu_keys_combo =  menu_keys.addItem(MenuKeyBind.new("Combo Key", 32))
         menu_keys_harass =  menu_keys.addItem(MenuKeyBind.new("Harass Key", string.byte("C")))]]
